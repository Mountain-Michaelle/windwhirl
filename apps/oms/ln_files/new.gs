...
// Changes this revision — three targeted fixes only, no column/structure
// changes, nothing that touches your existing data:
 ...
 *  - "Duplicate Order?" is set automatically when phone number, product,
 *    and address all exactly match another row already in the sheet.
...
function processRow(sheet, row, rawText) {
  ...
  for (let i = 1; i < blocks.length; i++) {
    ...
    // earlier/pre-existing order could get its TRUE flag written to the WRONG
...
function writeRecord(sheet, row, blockText, scanData) {
  ...
  const isDuplicate = scanData
    ? findDuplicateInScanData_(sheet, scanData, row, phone, productText, fields.address)
    : checkAndFlagDuplicates(sheet, row, phone, productText, fields.address);
...
// In-memory equivalent of checkAndFlagDuplicates() — same matching rule (exact
// phone + product + address match against another row), but checks against
// the scanData snapshot instead of re-reading the sheet. Still writes TRUE to
// a matching historical row when found — that part can't be avoided, since a
// newly-duplicated old row genuinely needs to be flagged on the sheet itself.
function findDuplicateInScanData_(sheet, scanData, row, phone, product, address) {
  ...
  for (let i = 0; i < scanData.rows.length; i++) {
    ...
    if (r.phone === targetPhone && r.product === targetProduct && r.address === targetAddress) {
      duplicateFound = true;
...
// Flags the current row TRUE if phone+product+address exactly match another
// row already in the sheet, and also flags that matching row TRUE.
function checkAndFlagDuplicates(sheet, row, phone, product, address) {
  sheet = sheet || SpreadsheetApp.getActiveSheet(); // guards against running this directly from the editor's function dropdown
  const lastRow = sheet.getLastRow();
  ...
  for (let i = 0; i < numRows; i++) {
    ...
    if (r === row) continue;
    if (normalizeText(phones[i][0]) === targetPhone &&
        normalizeText(products[i][0]) === targetProduct &&
